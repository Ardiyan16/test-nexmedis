...
class IsAdmin
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if(empty($token) || empty($user_type) || empty($user_id) || empty($code) || empty($exp_token)) {
            return response([
                'status' => false,
        ...
        if(strtotime('now') >= strtotime($cek_exp)) {
            return response([
                'status' => false,
                'message' => 'Token anda telah kadaluarsa! silahkan login kembali'
            ], Response::HTTP_UNAUTHORIZED);
        }
        ...
        if(empty($user)) {
            return response([
                'status' => false,
                'message' => 'User tidak ditemukan, silahkan login kembali'
            ], Response::HTTP_UNAUTHORIZED);
        }
